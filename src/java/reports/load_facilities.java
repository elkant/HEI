...
HttpSession session;
String districts,output,where,query;
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
           session = request.getSession();
            ...
            districts = request.getParameter("districts");
            
            String district_array[] = districts.split("_");
            
            for(String ct:district_array){
             if(!ct.equals("")){
                 where+=" district_id="+ct+" OR ";
             } 
            }
            
            where = removeLast(where, 3);
            output = "";
            query = "SELECT facility_id,facility_name FROM facilities WHERE ("+where+") ORDER BY facility_name";
